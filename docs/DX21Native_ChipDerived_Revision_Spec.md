...
PL50〜99はDX21実機録音から得た上方向テーブルを使う。
PL0〜49は5刻みの下方向実測アンカーを線形補間する。
...
double pitchEgLevelToCents(int pl)
{
    pl = clampInt(pl, 0, 99);
    if (pl >= 50)
        return kPitchEgUpperLevelCents[pl - 50];
    return interpolateLowerPitchEgLevelCents(pl);
}
```
...
下方向は概ね `PL50〜36 = -50 cent/step`, `PL35〜10 = -100 cent/step`, `PL9〜0 = -150 cent/step` の段階式に近い。
実装では測定アンカーに合わせるため、5刻みの間を線形補間する。
...
公式テーブルは確認できないため、DX21実機録音による測定値からPR→時間を近似する。
...
`PR = 0` と `PR = 90..99` は端点/実測補正として扱う。
`PR90..98` はC1/C3の録音測定から平均寄りの値を採用する。
`PR99` は録音上ほぼ即時に最終音程へ到達して聞こえるが、完全な0秒ジャンプではなく、DX21/OPM系のEG制御tickに合わせて `100 / (3579545 / 64 / 3)` 秒として扱う。
PR1 / PR2 / PR3 は同じPR→時間カーブを使う。
