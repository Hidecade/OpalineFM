...
- モード選択付きAuto Pan／Chorus、Stereo Delay、Reverbと個別パラメーター
- スタンドアロン版のWAV録音
...
デスクトップ版の画面鍵盤はC2から始まり、C音にオクターブ番号を表示します。
iPhone版の初期位置はC3です。Standalone版のPCキー配列はAurelineと共通で、
下段は`Z`=C2、上段は`Q`=F3から始まります。
...
初回起動時は`assets/factory.syx`を既定のファクトリーバンクとして読み込みます。ファイルを利用できない場合のみ、従来のファクトリーライブラリーXMLへフォールバックします。
...
- **AUTO PAN**: SINE、TRIANGLE、SQUARE、RANDOM、CHORUSからモードを選び、RATEとDEPTHを設定。
- **DELAY**: OFF、STEREO、PING PONG、ECHOからモードを選び、TIMEとMIXを設定。
- **REVERB**: OFF、ROOM、HALL、PLATEからモードを選び、SIZEとMIXを設定。
- **FX ON**: エフェクトチェーン全体を有効／無効にする。

iPhone版のFXコンボはメイン画面のコンボと同じスタイルで、各2ノブの上に配置されています。EDIT画面では編集パネルが上、波形と鍵盤が下に配置され、下段の幅は上のEDITパネルに揃えています。
