...
現在の公開リリースは**v1.0.6**です。Windows版とmacOS版は [v1.0.6 Release](https://github.com/Hidecade/OpalineFM/releases/tag/v1.0.6) からダウンロードできます。

使いたい形式に合わせて選んでください。

- `OpalineFM-Standalone-1.0.6.0-macOS.pkg`: 単体アプリとして使う場合。DAWなしでOpaline FMを演奏できます。
- `OpalineFM-AU-1.0.6.0-macOS.pkg`: Logic Pro、GarageBand、AU対応DAWで使う場合。
- `OpalineFM-VST3-1.0.6.0-macOS.pkg`: VST3対応DAWで使う場合。
- `OpalineFM-Standalone-v1.0.6-Windows-x64.exe`: 64-bit Windows用スタンドアロンアプリ。
- `OpalineFM-VST3-v1.0.6-Windows-x64.exe`: 64-bit Windows用VST3インストゥルメント。
