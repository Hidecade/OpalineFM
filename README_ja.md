...
- スタンドアロン版のWAV録音
- Windowsスタンドアロン/VST3とmacOS Audio Unitのビルド経路
...
右上のエンジンボタンでレンダリングモデルを切り替えます。

- **Type A**: 従来の互換Opalineレンダリング経路を維持します。
- **Type B**: よりチップ寄りのオペレーター、フィードバック、アッテネーション、出力経路を使用します。

表示される音色パラメーターは共通です。Type Bはチップ動作再現を進める主経路で、Type Aは比較にも利用できます。
...
Standaloneインストーラーを実行し、スタートメニューから**Opaline FM**を起動します。DAWは不要です。

### Windows VST3

VST3インストーラーを実行するか、`Opaline FM.vst3`フォルダー全体を次へコピーします。

```text
C:\Program Files\Common Files\VST3\
```

DAWでプラグインを再スキャンし、インストゥルメントトラックへOpaline FMを挿入します。

### macOS Audio Unit

`Opaline FM.component`をユーザー用またはシステム共通のComponentsフォルダーへコピーします。
...
DAWを再起動するかAudio Unitを再スキャンします。現在のリポジトリはAUビルド経路を提供し、署名・公証済みパッケージは別途配布できます。
...
WindowsインストーラーにはInno Setup 6または7が必要です。
