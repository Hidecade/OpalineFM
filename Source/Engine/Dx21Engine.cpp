...
StereoSample Dx21Engine::processEffects(const double input)
{
    ...
    const double mix = static_cast<double>(fx.mix) / 99.0;
    ...
    const double dryGain = 1.0 - mix * 0.55;
    const double wetGain = mix * (0.18 + reverb * 0.82);
    ...
    const double wetInLeft = input + reverbOutLeft;
    const double wetInRight = input + reverbOutRight;
    const double delayedLeft = delaySamples > 1.0 ? readDelay(delayBufferLeft, delayWriteIndex, delaySamples) : wetInLeft;
    const double delayedRight = delaySamples > 1.0 ? readDelay(delayBufferRight, delayWriteIndex, delaySamples) : wetInRight;
