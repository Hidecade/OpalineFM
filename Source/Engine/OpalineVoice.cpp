...
namespace
{
constexpr std::size_t kChipRomTableSize = 256;

// Generate the quarter-wave logarithmic attenuation curve from its mathematical
// definition instead of embedding values copied from another implementation.
const std::array<int, kChipRomTableSize> kGeneratedLogSinRom = []
{
    std::array<int, kChipRomTableSize> values {};
    for (std::size_t i = 0; i < values.size(); ++i)
    {
        const double phase = (static_cast<double>(i) + 0.5) * kPi / 512.0;
        values[i] = static_cast<int>(std::round(-std::log2(std::sin(phase)) * 256.0));
    }
    return values;
}();

// Generate the inverse logarithmic curve with the same quantization used by
// the operator output path.
const std::array<int, kChipRomTableSize> kGeneratedExpRom = []
{
    std::array<int, kChipRomTableSize> values {};
    for (std::size_t i = 0; i < values.size(); ++i)
    {
        const double exponent = static_cast<double>(255 - i) / 256.0;
        values[i] = static_cast<int>(std::round(std::exp2(exponent) * 1024.0));
    }
    return values;
}();
}
...
{
    return kGeneratedLogSinRom;
}

const std::array<int, 256>& chipExpRom()
{
    return kGeneratedExpRom;
}
}
...
{
    const auto& table = detail::chipLogSinRom();

    int quarterIndex = phaseIndex & 255;
    if ((phaseIndex & 256) != 0)
        quarterIndex ^= 255;

    return table[static_cast<std::size_t>(quarterIndex)];
}
...
{
    const auto& table = detail::chipExpRom();

    const int atten = clampInt(attenuation, 0, 4095);
    const int output = (table[static_cast<std::size_t>(atten & 255)] << 2) >> (atten >> 8);
